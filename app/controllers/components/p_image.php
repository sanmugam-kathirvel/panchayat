...
class PImageComponent extends Object 
{
	...
	function resizeImage($cType = 'resize', $id, $imgFolder, $newName = false, $newWidth=false, $newHeight=false, $quality = 85, $bgcolor = false)
	{
		//$img = $imgFolder . $id;
		$img = $id; // MODIFICATION
			...
			//{
				switch ($cType){
					...
					case 'resize':
						...
						if($newWidth) { 
						    if($newWidth > $oldWidth) $newWidth = $oldWidth;    $widthScale = $newWidth / $oldWidth; 
						} 
						if($newHeight) {
							if($newHeight > $oldHeight) $newHeight = $oldHeight;    $heightScale = $newHeight / $oldHeight;
						} 
